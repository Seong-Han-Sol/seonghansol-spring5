--19장 사용자추가시 오라클데스크탑X 대신
-- 웹프로그램을 사용 (http://127.0.0.1:9000/apex/f?p=4950
--,SQL플러스X,
--15장 PK생성시 자동으로 2가 생성 NOT NULL(빈값정지), UNIQUE(NO중복)
--제약조건(constraint)이 자동생성, Index도 자동생성(검색시중요)
--ERD로 게시판테이블 -댓글(첨부파일) Foreign Key(외래키) 부자관계 생성
--14장 트랙잭션 DB단에서 사용하지 않고,
...
--ERD관계형 다이어그램 물리적 테이블 생성(포워드엔지니어링)
...
--DEPT테이블의 deptno 숫자2자리때문에 에러 -> 4자리로 크기 변경
DESC dept; --단, 작은 자리에서 큰 자리로 이동은 문제X
