...
// 1번 서브메뉴 숨기는 함수(아래)
function goHide1() {
if(!isOver1 && !isOverSub1) { //2개 변수값이 false일때 구현조건이 됩니다.
	$('.gnb_depth2_1').stop().fadeOut('fast');
}
}
var isOver2 = false; //대메뉴 2번이 false 일때 서브메뉴가 사라지는 조건
var isOverSub2 = false; //서브메뉴 1번이 false 일때 서브메뉴가 사라지는 조건
// 2번 서브메뉴 숨기는 함수(아래)
function goHide2() {
...
$(document).ready(function(){
	...
	$('.openAll1').mouseout(function(){
		isOver1 = false;
		setTimeout('goHide1()',200); //기본은 goHide1() 이것만 하면 되는데, mouseout이벤트 놓치면 안 되기 때문에 setTimeout사용
	});
	//서브메뉴1 액션
	...
	$('.gnb_depth2_2').mouseover(function(){
		isOverSub2 = true; //서브메뉴 2이 계속 나타납니다.
	});
